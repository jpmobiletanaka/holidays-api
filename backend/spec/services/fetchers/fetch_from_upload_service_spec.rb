...
describe Fetchers::FetchFromUploadService do
  ...
  context 'when upload id found' do
    ...
    context 'when all countries exist' do
      ...
      context 'when creating holidays from file' do
        it 'saves all records without duplicates' do
          expect do
            expect do
              service.call(options: { upload_id: upload.id })
              upload.reload
            end.to change(upload, :status).from('pending').to('success')
          end.to change(FileRawHoliday, :count).by(18)
        end
      end
      ...
      #     inst.call
      #     expect(inst.send(:invalid_rows).size).to eq 8
      #   end
      # end
    end
  end
