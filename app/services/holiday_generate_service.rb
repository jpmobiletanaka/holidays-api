class HolidayGenerateService
  def initialize(holiday_expr, start_date: 1970, end_date: 2038, delete: false)
    ...
    holiday_expr.holidays.delete_all if delete
  end

  def call
    dates         = extract_dates
    holiday_attrs = dates.map! { |date| extract_attributes(date) }
    Holiday.import!(holiday_attrs, on_duplicate_key_ignore: true, validate: false)
  end

  def extract_attributes(date)
    date = date.julian if holiday_expr.julian?
    {
      date: date,
